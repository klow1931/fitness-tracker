...
vm.createContext(ctx);vm.runInContext(fs.readFileSync(path.join(__dirname,'../src/product/workout-history.js'),'utf8'),ctx);
for(const id of [123,'123','a05c-uuid',"x\" onclick=\"bad()"]){
 ...
 assert(!html.includes('<script>'));assert(!html.includes('<img'));assert(html.includes('deleteWorkout(this.dataset.workoutId)'));assert(html.includes('data-workout-id="'+escapeHtml(id)+'"'));
}
