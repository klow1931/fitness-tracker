...
test('replacement import previews changes, saves recovery, and restores prior state',async({page})=>{
 ...
 await page.evaluate(()=>showTab('tools'));await expect(page.locator('#recovery-snapshots')).toContainText('Before JSON import');page.once('dialog',d=>d.accept());await page.locator('#recovery-snapshots [data-restore-snapshot]').click();await expect.poll(()=>page.evaluate(()=>data.workouts[0].id)).toBe(before.workouts[0].id);
});
