...
test('duplicate is a new draft and edit/delete revisions can be undone',async({page})=>{
 ...
 await expect(page.locator('#workout-history')).toContainText('Edited workout');await page.locator('#workout-history details:has([data-workout-action="undo-change"]) > summary').click();page.once('dialog',d=>d.accept());await page.locator('[data-workout-action="undo-change"]').first().click();expect(await page.evaluate(id=>data.workouts.find(w=>w.id===id).exercises[0].sets[0].weight,original)).toBe(100);
 page.once('dialog',d=>d.accept());await page.locator(`[data-hist-id="${original}"] [data-workout-action="delete"]`).click();expect(await page.evaluate(id=>data.workouts.some(w=>w.id===id),original)).toBe(false);await expect(page.locator('#workout-history')).toContainText('Deleted workout');await page.locator('#workout-history details:has([data-workout-action="undo-change"]) > summary').click();page.once('dialog',d=>d.accept());await page.locator('[data-workout-action="undo-change"]').first().click();expect(await page.evaluate(id=>data.workouts.some(w=>w.id===id),original)).toBe(true);
});
