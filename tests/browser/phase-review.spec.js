...
test('supported per-lift progression is optional and retains the unmodified program',async({page})=>{
 await page.evaluate(()=>{for(const w of data.workouts)for(const e of w.exercises)if(e.exerciseId==='b')for(const set of e.sets)set.rpe=set.targetRpe-1;renderPhaseReview();});
