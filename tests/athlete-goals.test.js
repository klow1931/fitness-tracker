...
const old={schemaVersion:16,goals:[{id:'legacy',name:'Legacy goal'}],workouts:[{id:'old',date:'2026-09-01',exercises:[]}]};const migrated=Core.normalizeState(old);assert.equal(migrated.schemaVersion,20);assert.deepEqual(migrated.athleteGoals,[]);assert.deepEqual(migrated.goals,old.goals);assert.deepEqual(migrated.workouts,old.workouts);
