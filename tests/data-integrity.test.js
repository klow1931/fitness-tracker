...
assert.equal(JSON.stringify(original),snapshot,'migration must not mutate imported input');assert.equal(migrated.schemaVersion,23);assert.equal(migrated.releaseVersion,require('../package.json').version);
