...
- Competition-only capacity endpoints reuse `Core.capacityEvidence`. They require at least three dates spanning 14 days. Variation estimates are not mixed into them. Progression compares the best eligible competition estimate in the first and final phase weeks, not different weekdays (such as primary versus light exposure). These are estimates, not measured maxes or proven gains. No percentage gain or confidence score is generated.
...
4. **Progress:** when all fully matched exposures are at least 1 RPE below caps of 7+, and at or below caps of 6 (the minimum usable RPE here), recovery is reported usual, three or more competition-lift RPE-aware dates span at least 14 days, the final phase-week best capacity endpoint is not more than 1% below the first phase-week best, and the whole future phase fits the exercise-specific 85%-of-training-max ceiling, offer a 2.5% load increase rounded down to the existing kg increment. The increase must actually clear the increment; no automatic progression, no variation-to-competition inference, and no measured strength-gain claim.\n5. **Add one set (optional):** only when the existing rounded load progression cannot be made, the whole accumulation phase has three weeks of identical prescribed weekly sets and at least two weekly lift exposures, all linked work is completed without changed/time-limited sessions, all sets meet the RPE margin, competition-lift capacity endpoints meet the existing guard, reported recovery is usual, and every next-phase exposure can add one copy of its last prescribed set without exceeding four sets or the reported session-time budget (six additional minutes per set). This is a bounded workload proposal, not an estimate of the athlete's optimal sets or a proven response to additional volume. Frequency never changes automatically.\n6. **Gather:** explain missing evidence or manual-review needs. Saving with Keep records the findings without editing the schedule.
