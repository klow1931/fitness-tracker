(function(root,factory){if(typeof module==='object'&&module.exports)module.exports=factory(require('../core/loadnote-core'),require('./session-intent'));else root.LoadnoteBlocks=factory(root.LoadnoteCore,root.LoadnoteIntent);})(typeof globalThis!=='undefined'?globalThis:this,function(Core,Intent){
 ...
 function analyze(records,workouts,id,{asOf,knownAt,retrospective=false}={}){
  ...
  return {block:b,contextMode:retrospective?'retrospective':'as-recorded',workoutCount:ws.length,totalVolume:ws.length?ws.reduce((s,w)=>s+Core.calcVolume(w),0):null,averageRPE:Core.averageRPE(ws),durationDays:duration,trainingDays:days,frequencyPerWeek:frequency,observedFrequencyPerWeek:observedFrequency,dataCompleteness:b.dataCompleteness,adherence:prescription.prescriptionCoverage,completionRate:prescription.setCompletionRate,prescribedIntensity:prescription.averageTargetRpe,prs:null,prescription,
