...
(function(){
 ...
 function markup(report,scope){
   ...
   return `<p class="more-hint">Observational only: these are recorded intentions and subsequent exposures, not proof a choice caused a result.</p>
     ...
     <p class="more-hint">Follow-ups: ${s.pending} awaiting · ${s.expired} beyond ${report.horizonDays}-day window · ${s.overlapping} overlapping, excluded from independent outcome totals. Each lift/workout is counted once.</p>
     <div class="decision-performance-response">${['accept','modify','ignore'].map(key=>{const d=s.byResponse[key];return `<div><b>${title[key]} · ${d.observed} observed</b><span>Mean capacity: ${d.observed?signed(d.meanCapacityChangePct)+'%':'—'} · Mean RPE change: ${d.observed?signed(d.meanRpeChange):'—'}</span><small>${d.outcomes.improved} improved · ${d.outcomes.stable} stable · ${d.outcomes.declined} declined</small></div>`;}).join('')}</div>
     <details class="more-details"><summary>Recorded choice vs recommendation</summary><p class="more-hint">A Modify response records intended direction, not a verified training prescription or proof it was followed.</p>${Object.entries(s.overridePatterns).length?Object.entries(s.overridePatterns).map(([k,p])=>`<p><b>${esc(k)}</b> · ${p.count} observed · ${p.improved} improved / ${p.stable} stable / ${p.declined} declined</p>`).join(''):'<p>No observed directional overrides yet.</p>'}</details>
     <details class="more-details"><summary>Audit decision and workout evidence · ${rows.length}</summary><div class="decision-performance-rows">${rows.map(row=>{
      ...
      const status={'attributed':'Unique follow-up','overlapping':'Overlapping · excluded','awaiting-outcome':'Awaiting follow-up','window-ended':'Window ended'}[row.attribution];
      return `<article><div class="decision-performance-row-heading"><b>${esc(title[row.snapshot.lift]||row.snapshot.lift)} · ${esc(when)}</b><small>${esc(status)}</small></div><p>Recommended: ${esc(title[row.snapshot.decision]||row.snapshot.decision)} · ${esc(title[row.response])} · Intended: ${esc(choice)}</p><p>${o?`Next exposure ${esc(o.date)} · capacity ${signed(o.capacityChangePct)}% · RPE change ${signed(o.rpeChange)}`:'No usable next exposure in the observation window.'}</p><details><summary>Evidence and original recommendation</summary><p>${esc(row.snapshot.reason)}</p><p>Next exposure guidance: ${esc(row.snapshot.nextExposure)}</p><small>Baseline: ${esc(row.snapshot.evidence?.at(-1)?.date||'unknown')} · ${esc(row.snapshot.evidence?.at(-1)?.workoutId||'no workout ID')}</small>${o?`<p><small>Observed workout ID: ${esc(o.workoutId)} · Exercise ID: ${esc(o.exerciseId)}</small></p><button type="button" class="btn-secondary" data-performance-workout-date="${esc(o.date)}">View workout in Train</button>`:''}</details></article>`;
