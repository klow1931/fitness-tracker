...
(function(){
 ...
 function markup(report,scope){
   ...
   return `<p class="more-hint">Observational only: these are recorded intentions and subsequent exposures, not proof a choice caused a result.</p>
     ...
     <p class="more-hint">Follow-ups: ${s.pending} awaiting · ${s.expired} beyond ${report.horizonDays}-day window · ${s.missingBaseline} missing baseline · ${s.overlapping} overlapping, excluded from independent outcome totals. Each lift/workout is counted once. Comparable coverage excludes overlapping events and events with no usable baseline.</p>
     ...
     <details class="more-details"><summary>Audit decision and workout evidence · ${rows.length}</summary><div class="decision-performance-rows">${rows.map(row=>{
      ...
      const status={'attributed':'Unique follow-up','overlapping':'Overlapping · excluded','awaiting-outcome':'Awaiting follow-up','window-ended':'Window ended','missing-baseline':'Missing baseline · outcome unavailable'}[row.attribution];
