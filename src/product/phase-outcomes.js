...
(function(root,factory){
  ...
  const planSets=(p,id)=>(p?.plannedExercises||[]).filter(e=>e.exerciseId===id).flatMap(e=>e.sets||[]);
  ...
  function analyze(state,{asOf,now=new Date().toISOString()}={}){
    ...
    for(const review of reviews){
      ...
      for(const lift of LIFTS){
        const finding=review.findings[lift],id=finding.exerciseId,rows=[];
        ...
        for(const target of source){
          ...
          const row={date:target.date,sessionId:sid,workoutId:ws.length===1?ws[0].id:null,status:'unconfirmed',approvedSets:planSets(approved?.context.prescription,id).length,completedSets:0,withinCap:null,estimatedCapacityKg:null};
          ...
          const expected=planSets(approved.context.prescription,id),actual=(w.exercises||[]).filter(e=>e.exerciseId===id&&e.type!=='cardio'&&e.trackBy!=='duration').flatMap(e=>e.sets||[]);
          row.completedSets=actual.filter(s=>Number.isFinite(s.weight)&&s.weight>0&&Number.isInteger(s.reps)&&s.reps>0).length;
          if(!expected.length||expected.length!==actual.length||!actual.every((s,i)=>s.reps===expected[i].reps&&Number.isFinite(s.weight)&&Math.abs(s.weight-expected[i].weight)<=.02)){row.status='work-deviated';rows.push(row);continue;}
          ...
          row.estimatedCapacityKg=bestCapacity(actual);rows.push(row);
        }
