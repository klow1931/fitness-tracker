...
(function(){
 ...
 function render(report){
   if(!report)return '';
   const sums=report.summary,part=report.phaseFocus;
   const next=report.next?'Next: '+report.next.date+' · '+report.next.name:report.phase==='mock-meet'?'No programmed attempts; mock-meet date '+report.meetDate+'.':'No further scheduled cycle workout in the current view.';
   return `<div class="card phase-guidance-card" data-phase-guidance data-phase="${esc(report.phase)}">
     <div class="phase-guidance-head"><div><span class="eyebrow">MOCK-MEET CYCLE · ${esc(report.status)}</span><h3>${esc(report.cycleName)} · ${report.totalWeeks} weeks</h3></div><span class="phase-guidance-chip">Week ${report.week} / ${report.totalWeeks}</span></div>
     ...
     <p class="phase-guidance-meta">Mock meet: ${esc(report.meetDate)} · Current-week review: ${esc(report.nextReview)} · Phase ends: ${esc(report.phaseEnd)}</p>
