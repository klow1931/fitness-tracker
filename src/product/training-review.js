...
(function(root,factory){
  ...
  const exclusionLabels={'low-rpe':'Low-effort set (RPE below 6): logged, not used to estimate capacity','missing-rpe':'RPE not recorded','invalid-rpe':'RPE outside the supported range','high-reps':'More than 12 reps','submax-single':'Submaximal single: observed load only, not an RPE-adjusted capacity estimate','invalid-set':'Invalid load or reps'};
  function review(state,{asOf,weeks=12,retrospective=true,knownAt,blockId}={}){
    ...
    const selectedBlock=blockId?Blocks.list(state.trainingBlocks||[],retrospective?undefined:cutoff).find(b=>b.id===blockId):null;
    if(blockId&&(!selectedBlock||selectedBlock.startDate>asOf))throw Error('Selected block is not available on this analysis date in this mode.');
    const from=selectedBlock?selectedBlock.startDate:move(asOf,1-weeks*7);
    const through=selectedBlock?.endDate&&selectedBlock.endDate<asOf?selectedBlock.endDate:asOf;
    ...
    const workouts=all.filter(w=>w.date>=from&&w.date<=through);
    ...
    const scope={id:'review-window',createdAt:'0001-01-01T00:00:00.000Z',updatedAt:'0001-01-01T00:00:00.000Z',
      revisions:[{recordedAt:'0001-01-01T00:00:00.000Z',context:Blocks.context({name:'Review window',startDate:from,endDate:through})}]};
    ...
    for(const w of workouts){
      ...
      for(const e of w.exercises||[])if(strength(e))for(const s of e.sets||[]){
        ...
        const capacity=Core.capacityEvidence(weight,reps,raw);
        ensure(e).actualSets.push({weight,reps,rpe:Number.isFinite(rpe)?rpe:null,
          estimatedCapacity:capacity.estimate,exclusion:capacity.reason,
          limitation:exclusionLabels[capacity.reason]||null});
      }
    ...
    const exercises=[...groups.values()].map(group=>{
      ...
      const exclusions={};for(const set of actual)if(set.exclusion)exclusions[set.exclusion]=(exclusions[set.exclusion]||0)+1;
      for(const [reason,count] of Object.entries(exclusions))reasons.push(count+' sets: '+exclusionLabels[reason]+'.');
      ...
      return {...group,contexts:undefined,actualSets:actual.length,usableSets:usable.length,exclusions,
    ...
    return {version:2,from,through,asOf,weeks,selectedBlock:selectedBlock?{id:selectedBlock.id,name:selectedBlock.name,startDate:selectedBlock.startDate,endDate:selectedBlock.endDate}:null,mode:retrospective?'current-corrected':'as-recorded',knowledgeCutoff:cutoff,
