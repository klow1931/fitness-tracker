(function(root,factory){
  ...
  const LIFTS=Builder.LIFTS,POLICY='phase-effort-v1';
  function analyze(state,{programId,phase,asOf,recovery,now=new Date().toISOString()}={}){
    ...
    for(const lift of LIFTS){
      ...
      let complete=true,comparedSets=0,overCapSets=0,overCapSessions=0,plannedSets=0,actualSets=0,totalVolumeKg=0,rpeSum=0,rpeCount=0;const performance=[];
      for(const row of rows){
        ...
        for(const e of planned){
          ...
          for(let i=0;i<e.sets.length;i++){
            ...
            comparedSets++;if(r>=t.targetRpe+1){over=true;overCapSets++;}
      ...
      let decision='keep',reason='Matched work stayed within this review rule. Keeping retains the planned next-phase progression; it does not establish that an increase was earned.';
      ...
      else if(['sleep','fatigue','soreness'].some(k=>ci[k]==='worse')){decision='reduce-sets';reason='Matched work stayed within caps, but recovery was reported worse. An optional reduction of one final working set per exposure is available; this is a conservative heuristic, not proof of excessive volume.';}
      const days=[...new Set(performance.filter(p=>Number.isFinite(p.estimatedCapacity)).map(p=>p.date))].sort();
      if(decision==='reduce-sets'&&main.sets<3){decision='gather';reason='Recovery needs review, but removing a set would leave fewer than two working sets. Review manually.';}
      // Repeated RPE-aware competition observations; never pool variations into capacity.
  ...
  function choicesValid(report,choices){for(const l of LIFTS)if(!['keep','reduce-load','reduce-sets'].includes(choices?.[l])||choices[l]!=='keep'&&choices[l]!==report.findings[l].decision)throw Error('Choose only a supported adjustment for each lift');}
  ...
  function validate(records){
    ...
    return records.map(r=>{
      ...
      for(const l of LIFTS){const f=r.findings[l];if(!f||!['keep','gather','reduce-load','reduce-sets'].includes(f.decision)||typeof f.reason!=='string'||typeof f.exerciseId!=='string'||!Array.isArray(f.performance)||!Number.isInteger(f.comparedSets)||f.comparedSets<0)throw Error('Invalid phase finding');}
  ...
  function apply(state,report,choices,{confirmed=false,asOf,now=new Date().toISOString(),lockedSessionIds=[]}={}){
    ...
    const event={version:1,policy:POLICY,id:Core.createId(),programId:report.programId,phase:report.phase,asOf,through:report.through,createdAt:now,recovery:report.recovery,findings:report.findings,evidence:report.evidence,choices:copy(choices),incrementKg:report.basis.program.config.incrementKg,exerciseLifts,changes};
