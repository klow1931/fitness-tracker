...
(function(){
  ...
  function exerciseKey(e){return e.exerciseId?'id:'+e.exerciseId:'name:'+String(e.name||'').trim().toLowerCase();}
  function selectedExercises(host,exercises){
    const select=host.querySelector('#review-exercises');if(!select)return exercises;
    const selected=[...select.selectedOptions].map(o=>o.value);
    if(!selected.length||selected.includes('all'))return exercises;
    return exercises.filter(e=>selected.includes(exerciseKey(e)));
  }
  function syncExercisePicker(host,exercises){
    const select=host.querySelector('#review-exercises');if(!select)return;
    const chosen=new Set([...select.selectedOptions].map(o=>o.value));
    select.innerHTML='<option value="all">All strength exercises</option>'+exercises.map(e=>'<option value="'+esc(exerciseKey(e))+'">'+esc(e.name)+'</option>').join('');
    if(chosen.size&& !chosen.has('all'))[...select.options].forEach(o=>o.selected=chosen.has(o.value));
    if(![...select.selectedOptions].length)select.options[0].selected=true;
  }
  function render(){
    ...
    if(!host.querySelector('#review-as-of')){
      host.innerHTML='<details id="training-review-panel"><summary><span><b>Training Review</b><small>Planned work, performance trends, and supporting evidence</small></span></summary><div class="training-review-body"><p>Choose the time period and exercises you want to inspect.</p><div class="training-review-controls"><label>Analyze through<input id="review-as-of" class="input" type="date"></label><label>Performance range<select id="review-weeks" class="input"><option value="1">1 week</option><option value="4">4 weeks</option><option value="12" selected>12 weeks</option></select></label><label>Training block<select id="review-block" class="input"><option value="">Rolling date range</option></select></label><label>Evidence mode<select id="review-mode" class="input"><option value="corrected">Current-corrected history</option><option value="recorded">Historical as-recorded</option></select></label><label>Exercises to show<select id="review-exercises" class="input" multiple size="4"><option value="all" selected>All strength exercises</option></select><small>Choose one or more. Leave All selected to show everything.</small></label><button id="review-refresh" class="btn-secondary" type="button">Refresh review</button></div><p id="review-error" role="alert"></p><div id="training-review-results"></div></div></details>';
      host.querySelector('#review-as-of').value=today();
      host.querySelectorAll('input,select').forEach(el=>el.onchange=render);
      host.querySelector('#review-exercises').onchange=()=>{const s=host.querySelector('#review-exercises');if([...s.selectedOptions].some(o=>o.value!=='all'))s.options[0].selected=false;render();};
    ...
    try{
      ...
      const week=r.week,c=week.counts;syncExercisePicker(host,r.exercises);const shown=selectedExercises(host,r.exercises);
      results.innerHTML='<p class="review-mode-note">'+esc(r.mode==='current-corrected'?'Current-corrected: uses today’s corrected workout and block context, excluding workouts after the analysis date.':'Historical as-recorded: reconstructs retained workout history and withholds later block context.')+'</p>'+
        ...
        (shown.map(e=>'<details class="review-lift"><summary>'+esc(e.name)+' · '+e.capacityDays+' capacity-evidence days</summary><dl><dt>Prescription progression</dt><dd>'+trend(e.prescriptionTrend)+'</dd><dt>Logged-load progression</dt><dd>'+trend(e.loggedLoadTrend)+'</dd><dt>Demonstrated-capacity estimate</dt><dd>'+trend(e.estimatedCapacityTrend)+'</dd></dl><p>'+e.usableSets+' / '+e.actualSets+' logged strength sets usable for capacity estimates.</p>'+
          ...
          e.evidence.map(row=>'<article class="review-evidence"><h5>'+esc(row.date+' · Workout '+row.workoutId)+'</h5><p><b>Planned:</b> '+sets(row.plannedSets,true)+'</p><p><b>Actual:</b> '+sets(row.actualSets,false)+'</p><p><b>Set capacity estimates:</b> '+(row.actualSets.length?row.actualSets.map(s=>s.estimatedCapacity==null?esc(s.limitation):esc(kg(s.estimatedCapacity))).join(' / '):'Not enough data')+'</p>'+
            ...
            (row.deviationReason!=='none'||row.deviationNotes?'<p>Change context: '+esc((LoadnoteIntent.DEVIATION_REASONS[row.deviationReason]||row.deviationReason)+' · '+row.deviationNotes)+'</p>':'')+'</article>').join('')+'</details>').join('')||'<p>No matching exercise evidence in this range.</p>')+
