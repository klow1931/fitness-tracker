...
(function(){
  ...
  function render(){
    ...
    try{
      ...
      const r=LoadnoteReview.review(data,{asOf,weeks:Number(host.querySelector('#review-weeks').value),retrospective:host.querySelector('#review-mode').value==='corrected'});
      ...
      results.innerHTML='<p class="review-mode-note">'+esc(r.mode==='current-corrected'?'Current-corrected: uses today’s corrected workout and block context, excluding workouts after the analysis date.':'Historical as-recorded: reconstructs retained workout history and withholds later block context.')+'</p>'+
        ...
        '<h3>Performance review · '+esc(r.from)+' – '+esc(r.asOf)+'</h3><p>'+r.workoutCount+' logged workouts. Planned-set completion: '+pct(r.execution.setCompletionRate)+'. This is separate from scheduled-session adherence.</p>'+
        ...
        '<p class="review-mode-note">Trends use the highest value per day and compare first/last evidence days, not a fitted growth rate. Capacity requires positive load, 1–12 reps and actual RPE 6–10. Training maxes and known 1RMs never fill missing estimates. Variations stay separate.</p>'+
